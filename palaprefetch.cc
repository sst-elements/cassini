...
void PalaPrefetcher::DispatchRequest(Addr targetAddress) {
    ...
    if (overrunPageBoundary) {
        output->verbose(CALL_INFO, 2, 0,
                        "Issue prefetch, target address: %" PRIx64 ", prefetch address: %" PRIx64
                        " (reach out: %" PRIu32 ", stride=%" PRIu32 "), prefetchAddress=%" PRIu64
                        "\n",
...
PalaPrefetcher::PalaPrefetcher(ComponentId_t id, Params &params) : CacheListener(id, params) {
    ...
    output->verbose(CALL_INFO, 1, 0,
                    "PalaPrefetcher created, cache line: %" PRIu64 ", page size: %" PRIu64 "\n",
                    blockSize, pageSize);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// This is loosely base on the work done by Palacharla and Kessler. The prefetcher maintains a
/// table indexed by a tag. The table contains the current stride value as well as the previous
/// address and previous stride value. The stride value is updated when the previous stride matches
/// for two fetches in a row. The default stride is the size of a cache line (initial value). The
/// table can hold a number of entries equal to address_count.
///
/// S. Palacharla and R. E. Kessler. 1994. Evaluating stream buffers as a secondary cache
/// replacement. In Proceedings of the 21st annual international symposium on Computer architecture
/// (ISCA '94). IEEE Computer Society Press, Los Alamitos, CA, USA, 24-33.
/// DOI=http://dx.doi.org/10.1145/191995.192014
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
