...
class StridePrefetcher : public SST::MemHierarchy::CacheListener {
   public:
    ...
    SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(StridePrefetcher, "cassini", "StridePrefetcher",
    ...
    SST_ELI_DOCUMENT_PARAMS(
        ...
        {"overrun_page_boundaries",
         "Allow prefetcher to run over page boundaries, 0 is no, 1 is yes", "0"})

    SST_ELI_DOCUMENT_STATISTICS(
        {"prefetches_issued", "Number of prefetch requests issued", "prefetches", 1},
        {"prefetches_canceled_by_page_boundary",
         "Prefetches which would not be executed because they span over a page boundary.",
         "prefetches", 1},
        {"prefetches_canceled_by_history",
         "Prefetches which did not get issued because of a prefetch history in the table",
         "prefetches", 1},
   ...
   private:
    ...
    auto getAddressByIndex(uint32_t index) -> Addr;
